...
现在谁看着它:`run_eval.py --selftest` 的第 8/9 组、四条 `kb-retention-*` / `retention-verdict-*`
负测、L7 金标 `L5-retention-cohort`(判分模式 `retention`,**先验结论再比数值**;数值不命中时
理由必须指向数值,否则修的人会去改文案)。写自测时它自己出了三个问题,都值得记:
