...
- **为什么不能用各表自己的 max()**：有几张表的轴伸出业务日历之外。用它们自己的 max 当锚点，窗口会落进一段只有零星数据的"未来"，**查得出数、不报错、数是错的**：
...
- 挑时间列要挑**业务时间**，不是 ETL 时间：`created_at` / `updated_at` / `attributed_at` 记的是"什么时候写进库的"，拿它们切窗口切出来的不是业务口径。
...
- **问句没点明时间范围时，一律按全量算**（`call_metric` 用 `time_window="all"`；写 SQL 就不要加「最近 N 天」这类相对窗口），不要自己替用户默认成「近 30 天」。「一共 / 总共 / 累计 / 总额 / 至今 / 历史上」这类词就是要全量；「这段时间」也是全量（指这份数据覆盖的整段），不是最近一个月。
  ...
  - 为什么较真：静默收窄窗口是这套系统里最典型的错法——**不报错、数看着正常、量级差好几倍**。「一共退了多少钱」全量是 **96.36 万**，默认成近 30 天给的是 **28.11 万**，差 3.4 倍；答案里还会写一句「我按近 30 天来算」，读的人多半不会去质疑这个前提。
