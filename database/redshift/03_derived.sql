-- ⚙️ 由 scripts/gen/pg_to_redshift.py 从 database/10_derived.sql 转换而来
...
od AS (
  SELECT user_id, placed_at::date AS dt,
         count(*) AS order_cnt,
         sum(CASE WHEN status IN ('paid','shipped','delivered') THEN actual_amount END) AS paid_amount
...
SELECT placed_at::date AS dt,
       sum(CASE WHEN status IN ('paid','shipped','delivered') THEN actual_amount END)::numeric(14,2) AS gmv,
       count(CASE WHEN status IN ('paid','shipped','delivered') THEN 1 END)          AS paid_orders,
       count(DISTINCT CASE WHEN status IN ('paid','shipped','delivered') THEN user_id END) AS paying_users,
       count(*) AS all_orders
FROM orders
GROUP BY 1;
