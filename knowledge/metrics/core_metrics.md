...
⚠️ **按天分 cohort 在这份数据上样本太小**：500 个用户摊到 30 天，每天只有 2–12 人，
实测某天 3/5 = **60% 次日留存**、隔一天又是 0%，全是分母噪声。要报数字请用下面的
**按周** cohort（每组 27–49 人），或者至少把 `cohort_size` 一起给出来。
...
>
> ⚠️ **这份数据的留存结论不可用**：活跃度与注册生命周期独立抽样，曲线**不衰减**。
> 实测（观测窗完整的 cohort）第 0→4 周 `45.0% / 43.0% / 42.1% / 41.1% / 43.0%`，
> 近乎水平且末周回升。**平坦不等于"留得住"**，别把它写成正面业务结论；
> 数字可以给，但要同时说明这份数据算不出留存。完整口径与结论写法见
> `analysis/retention_curve.md`——**答留存题请连它一起读**，哪怕只是取数。
