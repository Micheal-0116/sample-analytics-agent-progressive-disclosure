...
What is *not* in v3: the 80M-row generator path (`scripts/gen/` → Parquet → COPY), and **value-level masking** — Lake Formation has no such primitive; that one was a Redshift feature. The lakehouse loads the committed CSV seed data (~190k rows in the base tables) via `scripts/lakehouse/load.py`. v2's design and migration pitfalls are archived in [docs/architecture-v2-redshift-glue.md](docs/architecture-v2-redshift-glue.md); the v1 PostgreSQL path (local Docker container) still runs but is **legacy — kept, not maintained**; see [docs/legacy.md](docs/legacy.md).
...
- **An S3 table bucket loaded with the sample data, federated into Glue, plus an Athena workgroup** — DDL in `database/iceberg/`, one-time setup and loading in `scripts/lakehouse/` (`setup.py` creates the table bucket `analytics-agent-tables`, the namespace `app_analytics` and the workgroup `analytics-agent-wg`; `load.py` loads the CSVs). Athena bills per byte scanned, so there is nothing to leave running idle — but the workgroup's query result location is a normal S3 bucket you should include in your cleanup.
...
The default path runs against AWS resources you created, so tear them down when finished: delete the **S3 table bucket** (`analytics-agent-tables`, which deletes the Iceberg tables with it), the **Glue federated catalog entry** that points at it, the **Athena workgroup** (`analytics-agent-wg`) and its **query-result staging bucket**, plus the governance layer's **IAM role** (`analytics-agent-ro`) and the **Lake Formation grants** issued to it. Athena has no idle cost — it bills per byte scanned — so the recurring charge here is S3 storage, not compute. For the **cloud web deployment**, additionally tear down the Fargate relay + ALB, the Cognito user pool, the site S3 bucket, and disable & delete the CloudFront distribution.
