...
这份文档按由浅入深的顺序把库过一遍：48 张表、约 22 万行、8 个业务域加治理层。
每一步都配可以直接跑的 SQL，贴的输出全部抄自实跑结果（2026-08-19，在现行湖仓
S3 Tables + Athena 上重跑）。
...
2. **email / phone 是明文**。v2 在 Redshift 上挂过动态脱敏策略，**湖仓化之后这层治理
   还没重新实现**：`scripts/test_all.sh` 把 L4 打印成「未实现」而不是跳过，agent 现在
   能读到全部 48 张表，包括 `user_messages`。规划中的做法是 Lake Formation data cell
   filter + Glue Catalog View。**这不是数据坏了，是治理层缺口，写在这里以免被误读。**
...
| ⚠️ PII 未脱敏 | email / phone 明文，`user_messages` 可读 | **治理层（L4）未在湖仓实现**，不是数据坏了 |
