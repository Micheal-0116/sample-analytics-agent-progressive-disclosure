...
>
> 当前部署是 Athena + S3 Tables,数据源是仓库里的 `data/csv/`,**48 张表约 22 万行**。
> 尤其注意:文末「修复后复审」那张表里被打勾的不变量,**在当前这份种子数据上多数并不成立**——
>
> | 本文（v2 修复后）声称 | 当前 22 万行种子数据实测 |
> |---|---|
...
> 加上这个约束以后即便事件量均匀,人数也会逐层掉——当前 22 万行实测
